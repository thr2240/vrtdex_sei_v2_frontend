...
export const PricePairLabel: React.FC<React.PropsWithChildren> = () => {
  ...
  const onTokenSwitch = useCallback(() => {
    if (router.query.token === PredictionSupportedSymbol.VRT) {
      router.query.token = PredictionSupportedSymbol.SEI
    } else if (router.query.token === undefined && token.symbol === PredictionSupportedSymbol.VRT) {
      router.query.token = PredictionSupportedSymbol.SEI
    } else if (router.query.token === undefined && token.symbol === PredictionSupportedSymbol.SEI) {
      router.query.token = PredictionSupportedSymbol.VRT
