...
const PredictionConfigProviders = ({ children }) => {
  ...
  const [selectedToken, setConfig] = useState(() => {
    ...
    if ([PredictionSupportedSymbol.SEI, PredictionSupportedSymbol.VRT].includes(initToken)) {
      return initToken
    }

    return PredictionSupportedSymbol.VRT
  })

  useEffect(() => {
    const upperToken = _toUpper(token as string) as PredictionSupportedSymbol

    if ([PredictionSupportedSymbol.SEI, PredictionSupportedSymbol.VRT].includes(upperToken)) {
